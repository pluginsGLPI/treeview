   ...
   function getNodesFromDb() {
      ...
      for ($n=1; $n<=count($nodes); $n++) {
         if ($nodes[$n-1] <= $max_id && $n <= $max_level) {
            ...
            while ($r = (method_exists($DB, 'fetchAssoc') ? $DB->fetchAssoc($result) : $DB->fetch_assoc($result))) {
               ...
               if (in_array($r['id'], $nodes)) {
                  ...
                  foreach (self::$types as $type) {
                     ...
                     if ($DB->numrows($result_1)) {
                        ...
                        while ($row = (method_exists($DB, 'fetchAssoc') ? $DB->fetchAssoc($result_location) : $DB->fetch_assoc($result_location))) {
                     ...
                     while ($r_1 = (method_exists($DB, 'fetchAssoc') ? $DB->fetchAssoc($result_1) : $DB->fetch_assoc($result_1))) {
                        // Item's name schema
